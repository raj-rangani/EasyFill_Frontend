...
class AccordianView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    var adapter: IAccordianAdapter? = null
        set(value) {
            field = value
            render()
        }
    ...
    private fun arrowDirection(index: Int): IAccordianAdapter.ArrowDirection {
        return if (selectedPosition == index) {
            Log.d("ddw", "NONE")
            IAccordianAdapter.ArrowDirection.NONE
        } else if (index < selectedPosition) {
            Log.d("ddw", "DOWN")
            IAccordianAdapter.ArrowDirection.DOWN
        } else {
            Log.d("ddw", "UP")
            IAccordianAdapter.ArrowDirection.UP
        }
    }
